...
var _validateCase = require('./validate-case.js');

var _validateCase2 = _interopRequireDefault(_validateCase);
  ...
  */
	...
	_createClass(Komet, [{
		...
		value: function task(config) {
			_validateCase2.default.execute('string', config.alias);
			_validateCase2.default.execute('string', config.entry);
			if (config.dependencies) {
				_validateCase2.default.execute('array', config.dependencies);
				config.dependencies.forEach(function (dep) {
					_validateCase2.default.execute('string', dep);
				});
			}
